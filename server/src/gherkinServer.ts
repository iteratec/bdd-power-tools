...
connection.onCompletion(
  (txtDocPos: TextDocumentPositionParams): CompletionItem[] => {
    ...
    if (doc) {
      let pattern = gherkin.repository.keywords.patterns.find(p =>
        p.name.endsWith(globalSettings.language),
      );
      const keywords = pattern
        ? pattern.match
        : gherkin.repository.keywords.patterns.find(p =>
            p.name.endsWith(defaultSettings.language),
          )!.match;
      const keywordsArray = keywords
        .substring(1, keywords.length - 1)
        .split('|');
      pattern = gherkin.repository.steps.patterns.find(s =>
        s.name.endsWith(globalSettings.language),
      );
      const steps = pattern
        ? pattern.match
        : gherkin.repository.steps.patterns.find(p =>
            p.name.endsWith(defaultSettings.language),
          )!.match;
      const stepsArray = steps.substring(1, steps.length - 1).split('|');
      let lineToPos = doc.getText(
        Range.create(
          Position.create(txtDocPos.position.line, 0),
          txtDocPos.position,
        ),
      );
      ...
      while (
        (keyword === stepsArray[3] || keyword === stepsArray[4]) &&
        newPos.line > 0
      ) {
        newPos.line--;
        lineToPos = doc.getText(
          Range.create(
            Position.create(newPos.line, 0),
            Position.create(newPos.line, 1000),
          ),
        );
        match = new RegExp(
          `^\\s*\\b(${stepsArray[0]}|${stepsArray[1]}|${stepsArray[2]})\\b`,
        ).exec(lineToPos);
...
connection.onDidChangeConfiguration(change => {
  ...
  globalSettings = (change.settings.bddPowerTools ||
    defaultSettings) as BddPowerToolsSettings;
